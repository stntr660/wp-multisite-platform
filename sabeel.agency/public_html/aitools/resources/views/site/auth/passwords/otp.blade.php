...
<div class="relative h-screen log-bg flex flex-col items-center pb-11 login-bg overflow-auto font-Figtree">
    ...
    <div class="relative bg-white dark:bg-[#3A3A39] rounded-3xl w-[350px] xs:w-[388px] sm:w-[506px] h-max px-4 sm:px-10 py-8 z-[2] mt-11">
        <p class="text-center text-24 font-bold text-color-14 dark:text-white">{{ __('OTP') }}</p>
        @php $msg = __('This field is required.'); @endphp
        <form action="{{ $route }}" method="get" class="button-need-disable signupForm">
            {!! csrf_field() !!}
            <label class="block mt-6">
                <input class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium" name="token" placeholder="{{ __('Enter OTP') }}" required oninvalid="this.setCustomValidity('{{ $msg }}')"/>
                @if (Session::has('resend'))
                    <a class="mt-2" href="{{ route("users.resend.code", ['email' => session('email')]) }}">{{ __('Resend Code') }}</a>
                @elseif (Session::has('password_reset'))
                    <a class="mt-2" href="{{ route("password.reset.resend", ['email' => session('email')]) }}">{{ __('Resend Code') }}</a>
                @endif
            </label>
            <button class="block w-full bg-color-14 dark:bg-white text-white dark:text-color-14 text-16 font-semibold py-3 flex justify-center items-center gap-3 rounded-xl text-center mt-6">{{ __('Continue')}}
