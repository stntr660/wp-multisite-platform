...
<div class="relative h-screen log-bg flex flex-col items-center pb-28 login-bg overflow-auto font-Figtree">
    ...
    <div class="relative bg-white dark:bg-[#3A3A39] rounded-3xl w-[350px] xs:w-[388px] sm:w-[506px] h-max px-5 sm:px-10 py-8 z-[2] mt-11">
        <p class="text-center text-24 font-bold text-color-14 dark:text-white">{{ __('Create your account')}}</p>
        @if(is_array($preference) && count($preference) > 0)
            ...
            <div class="flex items-center justify-center mt-5">
                <svg class="w-[39px] xs:w-[58px] sm:w-[116px] text-color-DF dark:text-color-47" height="1" viewBox="0 0 122 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <line y1="0.5" x2="122" y2="0.5" stroke="currentColor"/>
                </svg>
                <p class="text-color-14 dark:text-white text-14 px-2 text-center">{{ __('Or register with your email')}}</p>
                <svg class="w-[39px] xs:w-[58px] sm:w-[116px] text-color-DF dark:text-color-47" height="1" viewBox="0 0 122 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <line y1="0.5" x2="122" y2="0.5" stroke="currentColor"/>
                </svg>
            </div>
        ...
        <form action="{{ route('users.registration') }}" method="post" id="password-validate-submit" autocomplete="off">
            {!! csrf_field() !!}
            <input type="hidden" value="{{$encryptedKey}}" name="encryptedKey">
            <div class="flex gap-4 sm:gap-5">
                <div class="w-[203px]">
                    <label class="block mt-5">
                        <span class="block text-14 font-medium text-color-14 dark:text-white">{{ __('First Name')}}</span>
                        <input type="text" class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium" name="first_name" value="{{ old('first_name') }}" placeholder="{{ __('First Name')}}" required oninvalid="this.setCustomValidity('{{ $msg }}')" />
                    </label>
                </div>
                @includeIf('affiliate::layouts.includes.affiliate_hidden')
                <div class="w-[203px]">
                    <label class="block mt-5">
                        <span class="block text-14 font-medium text-color-14 dark:text-white">{{ __('Last Name')}}</span>
                        <input type="text" class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium" name="last_name" value="{{ old('last_name') }}" placeholder="{{ __('Last Name')}}" required oninvalid="this.setCustomValidity('{{ $msg }}')"/>
                    </label>
                </div>
            </div>
            <label class="block mt-6">
                <span class="block text-14 font-medium text-color-14 dark:text-white">{{ __('Email Address')}}</span>
                <input type="text" class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium" 
                value="{{ old('email') }}" 
                name="email" placeholder="{{ __('Email Address')}}" required oninvalid="this.setCustomValidity('{{ $msg }}')" readonly onfocus="this.removeAttribute('readonly');"/>
            </label>
            <label class="block mt-6">
                ...
                <div class="relative password-container">
                    <div>
                        <input type="password" class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium password-field password-validation" name="password" placeholder="{{ __('Password')}}" required oninvalid="this.setCustomValidity('{{ $msg }}')" readonly onfocus="this.removeAttribute('readonly');" />
                    </div>
                    <span class="password-validation-error block text-xs mt-1"></span>
            ...
            <p class="text-13 text-color-89 text-center font-medium mt-5">
                {!! __('By creating an account you agree to our :x.', [
                    'x' =>
                        "<a class='text-15 font-semibold underline' href='" .
                        (option('default_template_page', '') != null
                            ? route('site.page', ['slug' => option('default_template_page', '')['term_condition']])
                            : '#') .
                        "'>" .
                        __('terms & conditions') .
                        '</a>',
            ...
            <button class="block w-full bg-color-14 dark:bg-white text-white dark:text-color-14 text-16 flex justify-center items-center gap-3 font-semibold py-3 rounded-xl text-center mt-5 signup-btn">{{ __('Get Started')}}
