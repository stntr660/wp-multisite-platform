...
class ModuleServiceProvider extends ServiceProvider
{
    ...
    public function __construct($app)
    {
        ...
        $this->isConsole = $this->app->runningInConsole()
            || str_contains(url()->full(), 'install/database')
            || str_contains(url()->full(), 'install/seedmigrate')
            || str_contains(url()->full(), 'system-update')
            || str_contains(url()->full(), 'reset-data');
    }
