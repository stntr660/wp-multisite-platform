...
@section('page_title', __('Plagiarism'))
@section('css')
    ...
    <div
        ...
        <div class="px-5 6xl:px-[100px] 7xl:px-[140px] 8xl:px-[200px] 9xl:px-[261px]  pt-[76px] lg:pt-[80px] pb-5">
            <p class="text-color-14 text-24 font-semibold font-RedHat dark:text-white">
                {{ __('Detecting Plagiarism in AI-Generated Content') }}</p>

            <p class="text-color-89 text-[13px] leading-5 font-medium font-Figtree mt-2">
                {{ __(':x prevent Plagiarism AI Powered Text Comparisons with Online Databases.', ['x' => preference('company_name')]) }}
            </p>
            <div class="subscription-main grid gap-6 xl:grid-cols-2 grid-cols-1">
                <form class="mt-5" id="plagiarism-form">
                    @if($pageLeft)
                    ...
                    <div class="bg-white dark:bg-[#3A3A39] p-6 rounded-xl mt-5">
                        <div class="custom-dropdown-arrow font-normal text-14 text-[#141414] dark:text-white {{ count($aiProviders) <= 1 ? 'hidden' : '' }}">
                            ...
                            <select class="select block w-full mt-[3px] text-base leading-6 font-medium text-color-FFR bg-white bg-clip-padding bg-no-repeat dark:bg-[#333332] rounded-xl dark:rounded-2xl focus:text-color-2C focus:bg-white focus:border-color-89 focus:outline-none form-control" name="provider" id="provider">
                                @foreach ($aiProviders as $provider => $value)
                                    @php
                                        $providerName = str_replace('plagiarism_', '', $provider);
                                    @endphp
                        ...
                        <div class="flex flex-col mt-2">
                            ...
                            <textarea
                                ...
                                id="plagiarism-description"
                                placeholder="{{ __('Briefly write down the description of the content you want to check..') }}"
                                data-min-length="{{ __('Description should be at least 80 characters.') }}" minlength="80"
                                required oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')" rows="20"
                                name="plagiarism-description"></textarea>
                        </div>
                        ...
                        <div class="mt-6">
                            <button
                                ...
                                id="plagiarism-creation">
                                <span>
                                    {{ __('Scan the plagiarism') }}
                                </span>
                ...
                <div class="bg-white dark:bg-[#3A3A39] p-6 rounded-xl xl:mt-11">
                    <p class="text-color-14 font-Figtree text-20 font-semibold dark:text-white text-center mb-3">{{  __("AI Plagiarism Report") }}</p>
                    <div class="ai-plagiarism-graph flex justify-center items-center">
                        <div class="bar-overflow-background">
                            ...
                            <div class="flex justify-center items-center gap-2 mt-[120px] flex-wrap">
                                ...
                                <div class="bar-overflow">
                                    <div class="progress-bars">
                                        <div class="bar-inner">
                                            <div class="percentage-wrapper text-color-2C dark:text-white font-Figtree text-sm font-medium">
                    ...
                    <p class="text-color-14 font-Figtree text-18 font-semibold dark:text-white text-left mb-3 p-3 border border-color-89 dark:!border-color-47 mt-6 rounded-xl">
                         {{  __("Report Details") }}
                    </p>
...
@section('js')
    ...
    <script src="{{ asset('Modules/OpenAI/Resources/assets/js/customer/plagiarism.min.js') }}"></script>
@endsection
