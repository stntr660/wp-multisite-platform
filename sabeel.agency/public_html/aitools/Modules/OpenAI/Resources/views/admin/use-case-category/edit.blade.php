...
@section('page_title', __('Edit Use Case Category'))
            ...
            <div class="col-sm-12 m-t-20 form-tabs">
                ...
                <div class="card-block table-border-style tab-content">
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                        <div class="col-sm-12">
                            <form action="{{ route('admin.use_case.category.edit', ['id' => $useCaseCategory->id]) }}" method="post" class="form-horizontal" enctype="multipart/form-data" id="form" onsubmit="return formValidation()" novalidate>
                                <input type="hidden" value="{{ csrf_token() }}" name="_token" id="token">
                                <div class="form-group row">
                                    <label for="name" class="col-sm-2 col-form-label require">{{ __('Name') }}</label>
                                    <div class="col-sm-10">
                                        <input type="text" placeholder="{{ __('Name') }}" class="form-control inputFieldDesign" id="name" name="name" value="{{ $useCaseCategory->name }}" maxlength="191" oninvalid="this.setCustomValidity({{ __('This field is required.') }})" required>
                                    </div>
                                </div>

                                <input type="hidden" id="slug" name="slug" value="{{ $useCaseCategory->slug }}">

                                <div class="form-group row">
                                    <label for="description" class="col-sm-2 col-form-label require whitespace-nowrap">{{ __('Description') }}</label>
                                    <div class="col-sm-10">
                                        <textarea placeholder="{{ __('Description') }}" id="description" maxlength="1000" class="form-control" name="description" oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')" required>{{ $useCaseCategory->description }}</textarea>
                                    </div>
                                </div>
...
<script>
    ...
    var slug = "{{ $useCaseCategory->slug === 'others' ? true : false }}";
</script>
