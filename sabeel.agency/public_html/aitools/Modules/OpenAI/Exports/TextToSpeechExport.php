...
class TextToSpeechExport implements FromCollection, WithHeadings, WithMapping
{
    /**
    ...
    {
        return Audio::with(['user:id,name', 'user.metas'])->get();
    }
     ...
     */
    public function map($textToSpeechList): array
    {
        return[
            $textToSpeechList->prompt,
            $textToSpeechList->language,
            $textToSpeechList->gender,
            optional($textToSpeechList->user)->name,
            timeZoneFormatDate($textToSpeechList->created_at),
        ];
    }
}
