...
class GatewayController extends Controller
{
    ...
    /**
     * Gateway Controller Constructor
     */
    ...
    public function pay(Request $request)
    {
        ...
        if (moduleAvailable($request->gateway) && $this->helper->isModuleActive($request->gateway)) {
            ...
            if ( config('openAI.is_demo') && ($request->gateway === "directbanktransfer" || $request->gateway === "checkpayments") ) {
        ...
        if (moduleAvailable($request->gateway)) {
            try {
                ...
                if (!$this->helper->getPaymentLog($code)) {
                    return redirect()->back()->withErrors(__(':x does not exist.', ['x' => __('Order')]));
                }
    ...
    public function paymentCancelled(Request $request)
    {
        try {
            ...
            return redirect(route('frontend.index'))->withErrors($e->getMessage());
        }
    }
