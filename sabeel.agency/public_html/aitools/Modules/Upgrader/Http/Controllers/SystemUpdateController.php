...
class SystemUpdateController extends Controller
{
    ...
    public function upgrade(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            \Session::flash('info', __('Please make sure you have the latest upgrader version (V1.2.0 or above)  before proceeding with the update process.')
                . " <a href='https://techvill.net/artifism/plugins?product=artifism' target='_blank'> <i class='feather icon-external-link'>" . __('Latest upgrader') . "</i></a>"
            );
            ...
            $data['applicationVersion']  = config('artifism.file_version', env('ARTIFISM_VERSION', '1.0.0'));
    ...
    {
        $itemCode = config('artifism.item_code', '43288879');
        $version = config('artifism.file_version', env('ARTIFISM_VERSION', '1.0.0'));
        $apiEndpoint = config('artifism.server_url', 'https://support.techvill.org') . "/api/v1/version/check/{$itemCode}/{$version}"; // Replace with the API endpoint
        $newVersion = $version;
    ...
    {
        $itemCode =  config('artifism.item_code', '43288879');
        $apiEndpoint = config('artifism.server_url', 'https://support.techvill.org') . '/api/v1/version/download'; // Replace with the API endpoint
