...
@section('seo')
    ...
    <meta name="keywords" content="{{ preference('company_name') }}, Ai Content, OpenAi, {{ preference('company_name') }} Ai Content">
@endsection
@section('content')
    <!-- component -->
    <div class="dark:bg-color-29 font-Figtree mt-[-10px] pt-[56px]">
        @if (count($blogs) >= 1)
            <div class="mx-4 lg:mx-4 2xl:mx-32 6xl:mx-64 9xl:mx-[375px]">
                <nav class="leading-4 md:mb-5 mb-6" aria-label="Breadcrumb">
                    <ol class="list-none inline-flex">
                        <li class="flex items-center">
                            ...
                            <a class="text-color-89 dark:text-color-DF font-medium md:text-14 text-12 ml-2 blog-menu" href="{{ URL::to('/') }}">{{ __('Home') }}</a>
                            <p class="text-color-89 dark:text-color-DF font-medium md:text-14 text-12 ml-2 blog-menu">/ </p>
                        </li>
                        <li>
                            @if (request()->search)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2 blog-menu" aria-current="page">{{ __('Search Results') }}</a>
                            @elseif (request()->year)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2 blog-menu" aria-current="page">{{ __('Archives') . ' ' . request()->year }}</a>
                            @elseif ($blogCategory)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2 blog-menu" aria-current="page">{{ $blogCategory->name }}</a>
                            @else
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2 blog-menu" aria-current="page">{{ __('All Posts') }}</a>
                            @endif
                        </li>
                    </ol>
                </nav>
                    ...
                    <div class="md:flex justify-between w-full lg:p-0 lg:mt-4">
                        ...
                        <div class="md:w-4/6 lg:w-full">
                            @foreach ($blogs as $blog)
                                <div class="block lg:flex lg:mb-[50px] md:mr-[30px] lg:mr-0 blog-image-section">
                                    <div class="rounded-md image-width-height blog-image-width-height overflow-hidden">
                                        <div class="grows">
                                            <a href="{{ route('blog.details', ['slug' => $blog->slug]) }}">
                                                <img class="h-60 w-[340px] image-width-height object-cover neg-transition-scale blog-image-width-height" src="{{ $blog->fileUrl() }}" alt="{{ $blog->title }}">
                                            </a>
                                        </div>
                                    </div>
                                    <div class="flex justify-start lg:w-[494px] w-full lg:mx-[30px] items-center">
                                        <div class="md:my-5 my-[18px]">
                                            <div>
                                                <p class="text-color-2C dark:text-white md:text-13 text-12 -mt-0.5 font-medium ml-1 mr-5 user-details-blog">
                                                    <span class="font-normal text-color-89 dark:text-color-DF">{{ __('By') }}</span>
                                                    <span>{{ isset($blog->user) && !empty($blog->user->name) ? optional($blog->user)->name : __('Guest Author')}}
                                                    </span>
                                            ...
                                            <div class="mt-2.5 flex flex-col">
                                                <a href="{{ route('blog.details', $blog->slug) }}" class="word-breaks text-color-2C dark:text-white font-medium lg:text-22 md:text-18 text-15" title="{{ $blog->title }}">{{ trimWords($blog->title, 65) }}
                                                </a>
                                                <p class="text-color-89 dark:text-color-DF md:mt-3.5 mt-2.5 md:text-14 text-12 font-medium word-breaks">{{ trimWords($blog->summary, 120) }}</p>
                                            </div>
                                            <div class="md:mt-[13px] mt-2.5">
                                                <a href="{{ route('blog.details', $blog->slug) }}"
                                                    class="font-medium md:text-16 text-14 text-color-89 inline-flex items-center hover:text-color-2C dark:text-white process-goto relative"><span>{{ __('Read Now') }}</span>
                                                    <svg class="ml-2 relative mt-1 w-3 h-2 md:w-4 md:h-4 blog-menu neg-transition-scale"
                            ...
                            @endforeach
                            {{ $blogs->links('blog::frontend.partials.pagination') }}
                        </div>
                        {{-- right sidebar --}}
        ...
        @else
            <div class="mx-4 lg:mx-4 xl:mx-32 2xl:mx-64 py-40 3xl:mx-[375px]">
                <div>
                    <img class="items-center mx-auto object-cover" src="{{ asset('public/frontend/assets/img/blog/sorry.svg') }}" alt="{{ __('Image') }}">
                    <p class="text-center font-medium text-32 leading-10 text-color-14 dark:text-white mt-[30px]">
                        {{ __('We’re sorry!') }}</p>
                    <p class="text-center font-medium text-32 leading-10 text-color-14 dark:text-white">{{ __('Blog post are not available') }}</p>
                </div>
            </div>
        @endif
    </div>
