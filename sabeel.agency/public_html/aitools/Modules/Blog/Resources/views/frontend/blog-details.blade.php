...
@php
    ...
    $seoKeywords = preference('company_name') . ', ' . 'Ai Content' . ', ' . 'OpenAi' . ', ' . preference('company_name') . ' ' . 'Ai Content';
@endphp
...
@endsection
@section('content')
    <div class="dark:bg-color-29 font-Figtree mt-[-10px] pt-[56px]">
        <div class="mx-4 lg:mx-4 2xl:mx-32 6xl:mx-64 9xl:mx-[375px]">
            <nav class="text-gray-600 text-14" aria-label="Breadcrumb">
                <ol class="list-none p-0 flex flex-wrap md:inline-flex text-12 md:text-14 font-Figtree font-medium text-color-89 dark:text-color-DF leading-5">
                    <li class="flex items-center">
                        <svg class="-mt-1" width="13" height="15" viewBox="0 0 13 15" fill="none"
                        ...
                        <a class=" ml-2 blog-menu" href="{{ URL::to('/') }}">{{ __('Home') }}</a>
                        <p class="px-2">/</p>
                    </li>
                    ...
                    @if (isset($blog->title) && !empty($blog->title))
                        <li>
                            <a href="javaScript:void(0);" class="text-color-2C dark:text-white" aria-current="page" title="{{ $blog->title }}">{{ trimWords($blog->title, 65) }}</a>
                        </li>
                    @endif
                </ol>
            </nav>
            <main>
                ...
                <div class="md:flex justify-between w-full mt-5">
                    <div class="md:w-4/6 lg:w-[69%] 3xl:w-full md:mr-8 blog-details-part">
                        {{-- post cards --}}
                        <div>
                            <div class="left-side w-full h-[411px]">
                                <img src="{{ $blog->fileUrl() }}" class="rounded w-full object-cover lg:h-[411px] h-full neg-transition-scale" />
                            </div>
                            <div class="mb-10 md:mt-3.5 mt-[18px]">
                                <div>
                                    <p class="text-color-2C dark:text-white leading-15p md:text-13 text-12 font-Figtree font-medium ml-1 mr-5">
                                        <span class="font-normal text-color-89 dark:text-color-DF">{{ __('By') }}</span>
                                        <span>
                                        ...
                                        @if (isset($blog->created_at) && !empty($blog->created_at))
                                            <span class="text-color-89 dark:text-color-DF">{{ __('On') }}</span>
                                            <span class="text-color-2C dark:text-white">{{ formatDateTime($blog->created_at) }}</span>
                                        @endif
                                    </p>
                                </div>
                                <p class="text-color-2C dark:text-white break-all font-bold font-Figtree mt-2.5 md:text-32 text-18 mb-2">
                                    {{ $blog->title }}
                                </p>
                                <div class="unreset dark:text-white rtl:pr-5 2xl:rtl:pr-0">
                                    {!! $blog->description !!}</div>
                            </div>
                        </div>
                        @if (isset($blog->user) && !empty($blog->user))
                            <div class="lg:w-[833px] w-full lg:min-h-[152px] border rounded border-gray-2 box-border">
                                <div class="grid lg:grid-cols-4 grid-cols-2">
                                    <img class="h-24 w-24 my-7 object-cover rounded-full ml-7 blog-user-image neg-transition-scale" src="{{ optional($blog->user)->fileUrl() }}"
                                        alt="{{ __('Image') }}">
                                    <div class="mt-8 lg:-ml-12 blog-user-details">
                                        <p class="font-Figtree text-13 font-medium text-color-89 dark:text-color-DF">{{ __('Author') }}</p>
                                        <p class="font-Figtree font-medium text-18 mt-[2px] text-color-2C dark:text-white lg:w-[152px] break-words pr-5 lg:pr-0 author-name">
                                            {{ optional($blog->user)->name }}</p>
                                        <p class="font-Figtree text-13 font-medium mt-0.5 text-color-89 dark:text-color-DF">
                                            {{ optional($blog->user)->designation }}
                                        </p>
                                        <div class="flex my-3.5 cursor-pointer text-color-89 dark:text-color-DF">
                                            @if (optional($blog->user)->facebook)
                                                <a href="{{ optional($blog->user)->facebook }}">
                                                    <svg class="hover:text-color-2C dark:text-white transition ease-in-out delay-130 blog-social-icon mr-3 neg-transition-scale"
                                            ...
                                            @endif
                                            @if (optional($blog->user)->twitter)
                                                <a href="{{ optional($blog->user)->twitter }}">
                                                    <svg class="t-0.5 hover:text-color-2C dark:text-white transition ease-in-out delay-130 blog-social-icon mr-3 neg-transition-scale"
                                            ...
                                            @if (optional($blog->user)->instagram)
                                                <a href="{{ optional($blog->user)->instagram }}">
                                                    <svg class="mt-0.5 hover:text-color-2C dark:text-white transition ease-in-out delay-130 neg-transition-scale"
                                    ...
                                    <div class="border-l my-6 lg:block hidden border-line w-0 min-h-[100px] -ml-20 blog-user-border"></div>
                                    <p class="lg:block hidden break-all my-10 italic text-left font-Figtree text-15 font-medium text-color-89 dark:text-color-DF mr-[30px] lg:-ml-[250px] blog-user-des">
                                ...
                                <div class="lg:hidden block">
                                    ...
                                    <div class="text-center italic m-6 break-all font-Figtree lg:text-15 text-13 font-medium text-color-89 dark:text-color-DF">
                        ...
                        <div class="flex justify-end mt-8">
                            <div class="flex">
                                @if ($social['facebook'] || $social['whatsapp'] || $social['instagram'] || $social['instagram'] || $social['linkedin'])
                                    <p class="text-16 font-Figtree font-medium text-color-2C dark:text-white mr-3">{{ __('Share on') }}:</p>
                                @endif
                                <div class="flex text-color-89 dark:text-color-DF mr-1 share-button-parents">
                                    @if ($social['facebook'])
                                    <a href="https://www.facebook.com/sharer.php?u={{ urlencode(url()->current()) }}"
                                        target="_blank">
                                        <svg class="transition ease-in-out delay-130 hover:text-color-2C dark:text-white hover:border-color-2C border border-color-DF rounded-full cursor-pointer mr-[7px] share-button-one-svg neg-transition-scale"
                                    ...
                                    <a href="https://api.whatsapp.com/send?text={{ urlencode(url()->current()) }}"
                                        target="_blank">
                                        <svg class="hover:text-color-2C dark:text-white transition ease-in-out delay-130 hover:border-color-2C border border-color-DF rounded-full cursor-pointer mr-[7px] share-button-one-svg neg-transition-scale"
                                            xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                                            viewBox="0 0 25 25" fill="none">
                        ...
                        <div class="flex justify-between my-[30px] gap-3">
                            @if ($previousUrl)
                                <div class="flex flex-col items-start justify-center lg:justify-start">
                                    <a href="{{ route('blog.details', $previousUrl->slug) }}" class="previous-post-rtl flex relative arrow-hover pl-4 md:text-16 text-14 font-Figtree font-medium text-color-89 dark:text-color-DF hover:text-color-2C justify-center">
                                        <svg class="mt-1.5 mr-2 neg-transition-scale absolute w-3 h-2 lg:w-4 lg:h-2.5" width="15" height="10" viewBox="0 0 15 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <span class="ml-4 font-Figtree font-medium md:w-full w-[120px] pre-span">{{ __('Previous Post') }}</span>
                                    </a>
                                    <a href="{{ route('blog.details', $previousUrl->slug) }}"
                                        class="flex text-16 font-Figtree break-all font-medium text-color-2C dark:text-white mt-3">{{ trimWords($previousUrl->title, 25) }}</a>
                                </div>
                            @else
                                <a href="javaScript:void(0);"
                                    class="pointer-events-none flex text-16 font-Figtree font-medium text-color-89 dark:text-color-DF justify-center">
                                    <svg class="mt-1.5 w-3 h-2 lg:w-4 lg:h-2.5 mr-3 previous-post-rtl-svg neg-transition-scale" xmlns="http://www.w3.org/2000/svg" width="15"
                                        ...
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                            ...
                                            fill="#898989" />
                                    </svg> <span class="ml-2 font-Figtree font-medium md:w-full w-[120px] blog-menu">{{ __('Previous Post') }}</span></a>
                            @endif
                            @if ($nextUrl)
                                <div class="flex flex-col lg:items-end items-end lg:justify-end">
                                    <a href="{{ route('blog.details', $nextUrl->slug) }}"
                                        class="process-goto hover:text-color-2C dark:text-white relative justify-center text-color-89 font-medium lg:text-16 text-14 font-Figtree flex font-Figtree text-right">
                                        <span class="-ml-5 md:w-full w-[120px] next-post-span">{{ __('Next Post') }}</span>
                                        <svg class="ml-2 mt-1.5 neg-transition-scale blog-menu relative w-3 h-2 lg:w-4 lg:h-2.5 neg-transition-scale" width="15" height="10"
                                    ...
                                    <a href="{{ route('blog.details', $nextUrl->slug) }}" class="text-16 lg:text-right text-right font-Figtree font-medium text-color-2C dark:text-white mt-3">{{ trimWords($nextUrl->title, 25) }}</a>
                                </div>
                            @else
                            <a href="javaScript:void(0);" class="process-goto disabled relative justify-center text-color-89 dark:text-color-DF font-medium lg:text-16 text-14 font-Figtree flex font-Figtree text-right">
                                <span class="-ml-5 w-[120px] md:w-full next-post-span">{{ __('Next Post') }}</span>
                                <svg class="mt-1.5 relative w-3 my-1.5 h-2 lg:w-4 lg:h-2.5 ml-2 blog-menu neg-transition-scale" width="15" height="10"
                                    viewBox="0 0 15 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        d="M9.70696 0L8.29274 1.41421L10.5856 3.70711H0.999849C0.447564 3.70711 -0.000150681 4.15482 -0.000150681 4.70711C-0.000150681 5.25939 0.447564 5.70711 0.999849 5.70711H10.5856L8.29274 8L9.70696 9.41421L14.4141 4.70711L9.70696 0Z"
                                        fill="#898989" />
                        ...
                        @if (isset($relatedPosts) && !empty($relatedPosts) && count($relatedPosts) > 0)
                            <p class="lg:text-22 text-18 font-Figtree lg:mt-10 uppercase font-bold text-center leading-29p text-color-2C dark:text-white">{{__('Related posts')}}</p>
                            <div class="flex gap-x-5 md:gap-x-3 overflow-auto md:mt-[15px] mt-2 mb-[30px]">
                                @foreach ($relatedPosts as $relatedPost)
                                    <div class="relative w-full md:w-1/3 sm:mb-0 mb-6">
                                        <a href="{{ route('blog.details', $relatedPost->slug) }}">
                                            <div class="w-[245px] md:w-full">
                                                <img alt="content" class="rounded h-[141px] w-[275px] object-cover"
                                                    src="{{ $relatedPost->fileUrl() }}">
                                                <div class="absolute top-2.5 bg-opacity rounded-sm px-2 pt-0 pb-0.5 left-2.5 blog-date">
                                                    <p class="text-center text-15 font-bold font-Figtree  md:mt-0">
                                                        {{ date('d', strtotime($relatedPost->created_at)) }}</p>
                                                    <p class="text-center text-10 font-normal -mt-1.5 font-Figtree">
                                                        {{ date('M', strtotime($relatedPost->created_at)) }}</p>
                                                </div>
                                            </div>
                                        </a>
                                        <p class="text-12 font-Figtree pt-2.5 text-color-89 dark:text-color-DF font-medium">{{ optional($relatedPost->user)->name }}
                                        </p>
                                        <a href="{{ route('blog.details', $relatedPost->slug) }}"
                                            class="text-16 font-Figtree mt-px break-all cursor-pointer text-color-2C dark:text-white font-medium" title="{{ $relatedPost->title }}">
                                            {{ trimWords($relatedPost->title, 65) }}</a> <br>
                                        <a href="{{ route('blog.details', $relatedPost->slug) }}"
                                            class="font-Figtree process-goto font-medium text-14 text-color-89 inline-flex items-center hover:text-color-2C dark:text-white process-goto relative trans-2 ml-auto">
                                            {{ __('Read Now') }}
                                            <svg class="fill-current relative ml-2 blog-menu neg-transition-scale"
