...
class AdminSupportDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($tickets) {
                ...
                if ($this->hasPermission(['Modules\Ticket\Http\Controllers\TicketController@edit'])) {
                    $str .= '<a title="' . __('Edit :x', ['x' => __('Ticket')]) . '" href="' . route('admin.threadEdit', ['id' => $tickets->id]) . '" class="btn btn-xs btn-primary"><i class="feather icon-edit neg-transition-scale-svg "></i></a>&nbsp';
                }
                if ($this->hasPermission(['Modules\Ticket\Http\Controllers\TicketController@delete'])) {
                    $str .= '<form method="post" action="' . route('admin.ticketDelete', ['id' => $tickets->id]) . '" id="delete-ticket-' . $tickets->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . csrf_field() . '
                        <button title="' . __('Delete') . '" class="btn btn-xs btn-danger confirm-delete" type="button" data-id=' . $tickets->id . ' data-delete="ticket" data-label="Delete" data-bs-toggle="modal" data-bs-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('Ticket')]) . '" data-message="' . __('Are you sure to delete this?') . '">
            ...
            ->rawColumns(['action', 'id', 'subject', 'assignee', 'status', 'last_reply', 'date', 'name', 'priority_name'])
            ->make(true);
    }
    ...
    */
    ...
    {
        $threads = Thread::with(['customer', 'assignedMember', 'assignedMember.metas', 'customer.metas', 'threadStatus', 'receiver.metas', 'priority'])
    ...
    */
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'name', 'name' => 'customer.name', 'title' => __('Customer')])
