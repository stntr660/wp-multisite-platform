<section id="top" class="w-full px-6 pt-3 overflow-hidden bg-white xl:px-8 "
    data-tails-scripts="//unpkg.com/alpinejs" >
    
    @include('wpbox::landing.partials.topbar')
    ...
    <div class="container pt-10 mx-auto text-left md:text-center">
        ...
        <div class="relative max-w-4xl mx-auto">
            ...
            <h1
                ...
                <span class="">{{ __('wpbox.header_pt1')}}</span> <span
                    class="block w-full py-2 text-transparent bg-clip-text leading-12 bg-gradient-to-r from-green-400 to-purple-500 lg:inline"
                    data-primary="green-400">{{ __('wpbox.header_pt2')}}</span> <span class="">{{ __('wpbox.header_pt3')}} 🚀</span>
            </h1>

            <p
                class="mt-8 mb-10 text-sm text-left text-gray-500 sm:text-base md:text-lg md:max-w-xxl md:text-center md:mx-auto">
                {{ __('wpbox.header_subtitle') }}
            </p>
            <a href="{{ route('register') }}"
                class="relative w-full px-12 py-5 mt-10 text-xl font-semibold leading-5 text-center text-white capitalize bg-gray-900 md:w-auto hover:bg-gray-800 focus:outline-none focus:bg-gray-800 md:mx-0"
                data-rounded="" data-primary="gray-900">
                {{ __('wpbox.start_now') }}
            </a>
        </div>

        <div class="box-content relative w-full h-64 max-w-6xl pb-0 mx-auto mt-16 overflow-hidden rounded-b-none lg:pb-32 md:h-88 lg:h-96 rounded-2xl"
            data-rounded="rounded-2xl" data-rounded-max="rounded-full">
            <img src="{{ asset('uploads/default/wpbox/header.png')}}"
